...
int main(int argc, char **argv) {
  Config cfg = {NULL, NULL, 50, 50, 1024, 0, 0, 0, 0};
  int status = 0;
  ...
  int opt;
  while ((opt = getopt_long(argc, argv, "ho:w:e:a:jv", long_options, NULL)) != -1) {
    switch (opt) {
    case 'v':
      printf("pngextract version %s\n", VERSION);
      return 0;
    case 'h':
      printf("Usage: %s <in.png> -o <out.png> [options]\n", argv[0]);
      return 0;
    case 'o':
  ...
  // Figure out if they want to dump the output into a directory or use a specific file name pattern.
  ...
  // We need a clean black and white mask to figure out where the separate pieces (sprites) are located.
  ...
  // ImageMagick hands us back a 'labeled' image where every pixel's color matches its component ID.
  Image *labeled_image = ConnectedComponentsImage(mask_image, 4, exception);
  if (!labeled_image) {
      if (cfg.json_mode) printf("{\n \"status\": \"error\", \"message\": \"Failed to label components\"\n}\n");
      else printf("status: error\nmessage: Failed to label components\n");
      return 1;
  }

  // Time to scan through the pixels. We have to map out the bounding box for each piece manually.
  size_t width = labeled_image->columns;
  size_t height = labeled_image->rows;
  
  // We don't know exactly how many pieces there are yet, so we'll dynamically build out a linked list as we find new ones.
  CCObjectInfo *objects = NULL;
  
  for (ssize_t y = 0; y < (ssize_t)height; y++) {
      const PixelPacket *pixels = GetVirtualPixels(labeled_image, 0, y, width, 1, exception);
      if (!pixels) break;
      for (ssize_t x = 0; x < (ssize_t)width; x++) {
          ...
          if (!found) {
              ...
              found->area = 0;
              if (prev) prev->next = found;
              else objects = found;
          }
          
          // Expand the bounding box as we discover more pixels for this specific piece.
          if (x < found->width) found->width = x;
          if (x > (int)found->offset.width) found->offset.width = x;
          if (y < found->height) found->height = y;
          if (y > (int)found->offset.height) found->offset.height = y;
          found->area++;
          found->centroid.x += x;
          found->centroid.y += y;
      }
  ...
  // Alright, we mapped everything. Let's calculate the final width and height for each piece.
  CCObjectInfo *curr;
  ...
  int valid_total = 0;
  for (curr = objects; curr != NULL; curr = curr->next) {
    if (curr->id == bg_id) // The background is usually the biggest piece, let's skip it.
  ...
  if (valid_total >= 1) {
    ...
    for (curr = objects; curr != NULL; curr = curr->next) {
      ...
      if (is_dir) {
        // If they passed a folder path, we'll just name the files based on the input name.
        ...
        if (dot)
          *dot = '\0';
        snprintf(final_name, sizeof(final_name), "%s%s-%d.png", cfg.output_pattern, bname,
                 counter + 1);
      } else if (strstr(cfg.output_pattern, "%d")) {
        snprintf(final_name, sizeof(final_name), cfg.output_pattern, counter + 1);
      } else {
        char base[1024];
        ...
        if (dot)
          *dot = '\0';
        snprintf(final_name, sizeof(final_name), "%s-%d.png", base, counter + 1);
      }
      ...
      if (cfg.json_mode) {
        printf("    { \"filename\": \"%s\", \"dimensions\": [%d, %d], \"size\": \"%.1fkb\" }%s\n", 
